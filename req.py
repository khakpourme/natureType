...
def get_request(xLeftyTop, xLeftyBottom, xRightyBottom, xRightyTop):
    ...
    url1 = "https://arcgisproxy.miljodirektoratet.no/arcgis/rest/services/naturtyper_dn/MapServer/1/query?geometry={bbox}&geometryType=esriGeometryEnvelope&outFields=Naturtype&returnGeometry=True&f=json&MaxRecordCount={recordCount}&spatialRel=esriSpatialRelContains"
    ...
    polygonshapefilepath = createpoly.create_poly_shape() # Calling function to create polygon shapefile from training dataset json downloaded from miljodirektorat Api
    shapefilepath = getCentroid.main(polygonshapefilepath) # Calling function to create shapefile of centroids of polygon shapefile

    # Raster URL
    url = 'https://services3.geodataonline.no/arcgis/rest/services/Geomap_UTM33_EUREF89/GeomapSentinel/ImageServer/exportImage?f=image&format=tiff&bandIds=&renderingRule={rasterFunction}&bbox={bbox}&imageSR=32633&bboxSR=32633&size={size}'
    with open('renderingrule.txt') as f:
    ...
    for i in range(lenght):
        ...
        # size="1096%2C409"
        full_url = url.format(rasterFunction=renderingRuleList[i], size="3288%2C1227", bbox=str(xLeftyTop)+"%2C"+str(xLeftyBottom)+"%2C"+str(xRightyBottom)+"%2C"+str(xRightyTop))
        # print full_url
